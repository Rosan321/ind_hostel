...
import { getProfileById } from "@/lib/store/actions/profileActions";
import { getAllAccomodationStayType } from "@/lib/store/actions/accomodationActions";
import { AuthButton, NavLink } from "@/lib/utils/headerUtils";
import { DROPDOWN_CONFIGS, NAV_LINKS } from "@/lib/utils/navigationConfig";
...
const Header = () => {
  ...
  const handleDropdownItemClick = (type, category) => {
    console.log("Navigating to:", type, category); // debug — remove after fix confirmed
    const queryParams = new URLSearchParams({ type, category }).toString();
    const url = `/data?${queryParams}`;
  ...
  // Wrapper function for nested dropdown that adapts the item object to the original signature
  const handleNestedDropdownItemClick = (item) => {
    handleDropdownItemClick(item.type, item.category);
  };
        ...
        <div className="flex items-center justify-between py-4">
          ...
          <div className="hidden lg:flex items-center justify-end gap-5 flex-1">
            ...
            >
              <Link
                href="/"
                className={`relative xl:text-lg px-4 lg:px-0 xl:px-4 font-semibold py-2
                after:absolute after:left-0 after:bottom-0 after:h-[2px]
                after:bg-[#0D0BA8]
                after:transition-all after:duration-300
              ...
              {/* Single Find Hostels dropdown with nested categories */}
              {DROPDOWN_CONFIGS.map(({ type, label, categories }) => (
                <li
                  key={type}
                  className="relative group flex items-center gap-2 lg:gap-1 xl:gap-2 cursor-pointer select-none"
                  onMouseEnter={() => handleNavItemMouseEnter(type)}
                ...
                >
                  ...
                  />
                  {activeDropdown === type && categories && (
                    <NestedDropdownMenu
                      categories={categories}
                      onItemClick={handleNestedDropdownItemClick}
                      onMouseEnter={handleDropdownMouseEnter}
                      onMouseLeave={handleDropdownMouseLeave}
                    />
                  )}
