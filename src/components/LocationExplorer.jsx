                ...
                <button
                  ...
                  className={`flex-shrink-0 px-4 lg:px-6 py-2 lg:py-3 text-[#1A1A1A] rounded-full text-base font-semibold transition-all duration-300 cursor-pointer whitespace-nowrap ${
                    selectedCity === city.name
                      ? "bg-[#2A32FF] text-[#ffffff] shadow-lg"
                      : "border border-gray-300 hover:bg-[#2A32FF] hover:text-[#ffffff]"
                  }`}
                >
                  {city.name}
                </button>
          ...
          <div className="rounded-2xl">
            ...
            <div className="xl:space-y-6">
              {currentHostels.length > 0 ? (
                currentHostels.slice(0, 3).map((hostel, index) => (
                  <ShuffleInOnScroll
                    key={hostel.id}
                    delay={0.15 * index}
                    intensity={50}
                  >
                    <div className="flex lg:gap-4 gap-6 items-start transform transition duration-300 hover:scale-102">
                      ...
                      <div>
                        ...
                        <p className="text-xs lg:text-base font-bold text-[#2A32FF] my-2">
                          {hostel.price}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2 lg:mb-4">
                          {hostel.amenities.map((amenity, index) => (
                            <span
                              key={index}
                              className="bg-[#fff] border-1 border-[#E0E0E0] text-[#44475A] px-3 sm:px-5 py-1.5 sm:py-2 rounded-full text-[10px] lg:text-sm font-medium"
                            >
                              {amenity}
                            </span>
                          ))}
